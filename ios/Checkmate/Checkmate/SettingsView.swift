...
private struct LegacyContextsView: View {
  @Environment(AppModel.self) private var model
    ...
    List {
      ForEach(model.contexts) { context in
        Section {
          ...
          let contextTasks = model.tasks.filter { $0.contextId == context.id && $0.status.isOpen }
            .count
          LabeledContent("Open tasks", value: String(contextTasks))
        } header: {
          Label {
            Text(context.name)
          } icon: {
...
private struct LegacyRoutinesView: View {
  @Environment(AppModel.self) private var model
  var body: some View {
    List(model.recurrences.filter { $0.kind == .routine }) { item in
      VStack(alignment: .leading, spacing: 4) {
        Text(item.title)
        Text(
          [item.daySlot?.rawValue.capitalized, item.state.rawValue.capitalized].compactMap { $0 }
            .joined(separator: " · ")
        )
    ...
    .overlay {
      if model.recurrences.filter({ $0.kind == .routine }).isEmpty {
...
private struct LegacyActivityView: View {
  @Environment(AppModel.self) private var model
